...
export default function Page() {
  ...
  const { isPendingForMe, myLevel, refresh, dismiss } = useMyApprovalStatus(
    API_ENDPOINTS.FINANCE.JOURNAL_ACCOUNTING.MY_APPROVAL_STATUS,
    id,
    access.canApprove,
  );
  ...
  const actions = getPageActions({
    ...
    onDownload:        uuid ? () => window.open(`/print/journal-accounting/${uuid}`, "_blank") : undefined,
  ...
  return (
    <HeaderWrapper
      ...
      pendingApproval={
        isPendingForMe
          ? `Your approval is required at Level ${myLevel} for this Journal Accounting.`
          : null
      }
      onDismissApproval={isPendingForMe ? dismiss : undefined}
    >
      <JournalAccountingForm
        key={refreshKey}
        mode={access.mode}
        accountingId={id}
        onUuid={setUuid}
      ...
      <ApprovalActionModal
        ...
        actions={[
          { type: "approve", api: API_ENDPOINTS.FINANCE.JOURNAL_ACCOUNTING.APPROVE },
          { type: "reback",  api: API_ENDPOINTS.FINANCE.JOURNAL_ACCOUNTING.REBACK  },
          { type: "reject",  api: API_ENDPOINTS.FINANCE.JOURNAL_ACCOUNTING.REJECT  },
        ]}
      ...
      <HistoryTimelineSheet
        ...
        title="Journal Accounting History"
        api={API_ENDPOINTS.FINANCE.JOURNAL_ACCOUNTING.HISTORY}
        entityId={id}
      />
    </HeaderWrapper>
  );
