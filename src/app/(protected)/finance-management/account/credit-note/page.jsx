...
export default function Page() {
  ...
  const access = getPageAccess({ pageCode: "credit_note", pageType: "LIST" });
  ...
  useEffect(() => {
    ...
    apiRequest({ url: `${API_ENDPOINTS.FINANCE.CREDIT_NOTE.LIST}?projectCode=${projectCode}`, method: "GET" })
      .then((res) => {
      ...
      .catch(() => toast.error("Failed to fetch Credit Note list"))
  ...
  const columns = [
    ...
    { header: "CN No.",       accessor: "creditNoteNo",   width: "120px" },
    ...
    <HeaderWrapper header={<PageHeader actions={actions} />}>
      <div className="p-3 space-y-3">
        <SearchSection
          ...
          actions={
            access.canAdd
              ? [{ label: "+ Add Credit Note", onClick: () => router.push("/finance-management/account/credit-note/new") }]
              : []
          }
        />
        <DataTable
          ...
          data={filteredData}
          onRowClick={(row) => {
            if (access.canOpenDetails) router.push(`/finance-management/account/credit-note/${row._id}`);
          }}
        />
      </div>
    </HeaderWrapper>
