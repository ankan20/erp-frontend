...
export default function Page() {
  ...
  const { isPendingForMe, myLevel, refresh, dismiss } = useMyApprovalStatus(
    `${API_ENDPOINTS.PROJECT.SALE_CERTIFIED_BILL.MY_APPROVAL_STATUS}${id}/my-approval-status`,
    id,
    access.canApprove,
  );
  ...
  return (
    ...
    >
      <SaleCertifiedBillForm mode={access.mode} billId={id} onAfterSubmit={refresh} />
      ...
      <ApprovalActionModal
        ...
        actions={[
          { type: "approve", api: `${API_ENDPOINTS.PROJECT.SALE_CERTIFIED_BILL.APPROVE}${id}/approve` },
          { type: "reback",  api: `${API_ENDPOINTS.PROJECT.SALE_CERTIFIED_BILL.REBACK}${id}/reback`   },
          { type: "reject",  api: `${API_ENDPOINTS.PROJECT.SALE_CERTIFIED_BILL.REJECT}${id}/reject`   },
        ]}
        onSuccess={() => {
      ...
      <HistoryTimelineSheet
        ...
        title="Certified Bill History"
        api={`${API_ENDPOINTS.PROJECT.SALE_CERTIFIED_BILL.HISTORY}${id}/history`}
        entityId={id}
      />
    </HeaderWrapper>
  );
