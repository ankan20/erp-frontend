...
export default function Page() {
  ...
  const access = getPageAccess({ pageCode: "sale_certified_bill", pageType: "LIST" });
  ...
  useEffect(() => {
    ...
    const fetchList = async () => {
      try {
        ...
        toast.error("Failed to fetch Certified Bill list");
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="p-3">
        <SearchSection
          ...
          actions={
            access.canAdd
              ? [
                  { label: "+ New Sale Bill",   onClick: () => router.push("/project-management/customer-billing/sale-bill-certified/new") },
                ]
              : []
          }
        />
        <DataTable
          ...
          data={filteredData}
          onRowClick={(row) => {
            if (access.canOpenDetails) router.push(`/project-management/customer-billing/sale-bill-certified/${row.id}`);
          }}
        />
      </div>
    </HeaderWrapper>
