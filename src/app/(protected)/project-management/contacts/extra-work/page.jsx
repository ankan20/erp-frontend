...
export default function Page() {
  ...
  const access  = getPageAccess({ pageCode: "extra_work", pageType: "LIST" });
    ...
    const fetchList = async () => {
      try {
        const res = await apiRequest({
          url:    `${API_ENDPOINTS.PROJECT.EXTRA_WORK.LIST}?projectCode=${projectCode}`,
          method: "GET",
        });
        ...
        const list = (res.data || []).map((r, i) => ({
          id:             r.id,
          sl:             i + 1,
          orderNo:        r.extraWorkNo          || "",
          date:           getfmtDisplaydate(r.extraWorkDate) || "",
          orderTitle:     r.orderTitle           || "",
          totalAmount:    fmtNum(r.totalAmount),
          gstAmount:      fmtNum(r.gstAmount),
          basicAmount:    fmtNum(r.basicAmount),
          billed:         fmtNum(r.billed),
          jobBalance:     fmtNum(r.jobBalance),
        ...
        toast.error("Failed to fetch Extra Work list");
      } finally {
        setLoading(false);
      }
    };
  ...
  const columns = [
    ...
    { header: "GST",                     accessor: "gstAmount",      width: "110px", align: "right" },
    { header: "Basic",                   accessor: "basicAmount",    width: "110px", align: "right" },
    { header: "Billed",                  accessor: "billed",         width: "110px", align: "right" },
    { header: "Job Balance",             accessor: "jobBalance",     width: "120px", align: "right" },
    ...
    <HeaderWrapper header={<PageHeader actions={actions} />}>
      <div className="p-3">
        <SearchSection
          ...
          actions={
            access.canAdd
              ? [{ label: "+ New Extra Work", onClick: () => router.push("/project-management/contacts/extra-work/new") }]
              : []
          }
        />
        <DataTable
          ...
          data={filteredData}
          onRowClick={(row) => {
            if (access.canOpenDetails) router.push(`/project-management/contacts/extra-work/${row.id}`);
          }}
        />
      </div>
    </HeaderWrapper>
