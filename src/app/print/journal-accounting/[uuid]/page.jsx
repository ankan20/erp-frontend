...
async function downloadDocx(data, uuid, qrCanvasRef) {
  ...
  const headerTable = new Table({
    ...
    rows: [new TableRow({ children: [
      ...
      new TableCell({
        children: [
          new Paragraph({ children: [run("JOURNAL ACCOUNTING", { bold: true, size: 32 })] }),
  ...
  const cW = [0.05, 0.10, 0.33, 0.22, 0.15, 0.15].map((p) => Math.round(PAGE_W * p));
  ...
  const mainTable = new Table({
    ...
    borders: tblBorders,
    rows: [
      new TableRow({ tableHeader: true, children: ["Sl.", "CC Code", "CC Name & Description", "Short Description", "Orig. Amount (₹)", "Amount (₹)"].map((h, i) => tCell(h, cW[i], true, headShading, "center")) }),
      ...lines.map((l, idx) => new TableRow({ children: [
        tCell(String(l.slNo || idx + 1), cW[0], false, undefined, "center"),
        tCell(l.ccCode || "—",           cW[1], false, undefined, "center"),
        tCell(l.ccName || "—",           cW[2]),
        tCell(l.shortDescription || "—", cW[3]),
        numCell(l.originalAmount,        cW[4]),
        numCell(l.amount,                cW[5]),
      ]})),
      new TableRow({ children: [
        new TableCell({ children: [new Paragraph({ alignment: "right", children: [run("TOTAL", { bold: true })] })], columnSpan: 5, width: { size: cW.slice(0, 5).reduce((a, b) => a + b, 0), type: WidthType.DXA }, shading: totShading, margins: { top: 50, bottom: 50, left: 70, right: 70 } }),
        numCell(totalAmt, cW[5], true, totShading),
      ]}),
      new TableRow({ children: [new TableCell({ children: [new Paragraph({ children: [run("Amount in Words: ", { bold: true }), run(amountToWordsIN(totalAmt), { italic: true })] })], columnSpan: 6, width: { size: PAGE_W, type: WidthType.DXA }, borders: { left: tblBorder, right: tblBorder, top: noB, bottom: tblBorder }, margins: { top: 50, bottom: 50, left: 80, right: 80 } })] }),
    ],
  });
  ...
  const a    = document.createElement("a"); a.href = url; a.download = `JournalAccounting_${data.voucherNo || uuid}.docx`; a.click();
  URL.revokeObjectURL(url);
}

export default function JournalAccountingPrintPage() {
  ...
  useEffect(() => {
    if (!uuid) return;
    publicRequest({ url: `${API_ENDPOINTS.FINANCE.JOURNAL_ACCOUNTING.GET_BY_UUID}${uuid}` })
  ...
  return (
    <>
      <PrintTopBar
        title={`Journal Accounting — ${data.voucherNo || ""}`}
        ...
        <div className="bg-white max-w-[860px] mx-auto shadow-md print:shadow-none print:max-w-none" style={{ fontFamily: "var(--font-print), sans-serif" }}>
          ...
          <div className="flex items-center px-6 pt-4 pb-3">
            ...
            <div className="flex-1 flex items-center justify-center">
              <h1 className={`${SIZE.pageTitle} ${WEIGHT.bold} tracking-widest text-gray-900 uppercase`}>Journal Accounting</h1>
            </div>
          ...
          <div className="grid px-6 pb-3" style={{ gridTemplateColumns: "50% 50%" }}>
            <div className="space-y-0.5">
              ...
              <InfoRow label="Date"         value={fmt.date(data.voucherDate)} />
              <InfoRow label="JV No"        value={data.journalVoucherNo} />
              ...
              <table className="w-full border-collapse" style={{ tableLayout: "fixed" }}>
                ...
                <thead>
                  <tr className={COLOR.tableHeadBg}>
                    {["Sl.", "CC Code", "CC Name & Description", "Short Description", "Orig. Amount (₹)", "Amount (₹)"].map((h) => (
                ...
                <tbody>
                  {lines.map((l, idx) => (
                    <tr key={idx} className={idx % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                      <td className={`${B} px-2 py-2 ${SIZE.tableCell} text-center text-gray-500`}>{l.slNo || idx + 1}</td>
                      <td className={`${B} px-2 py-2 ${SIZE.tableCell} text-center ${WEIGHT.medium}`}>{l.ccCode || "—"}</td>
                      <td className={`${B} px-2 py-2 ${SIZE.tableCell}`} style={{ wordBreak: "break-word" }}>{l.ccName || "—"}</td>
                      <td className={`${B} px-2 py-2 ${SIZE.tableCell} text-gray-500`} style={{ wordBreak: "break-word" }}>{l.shortDescription || "—"}</td>
                      <td className={`${B} px-2 py-2 ${SIZE.tableCell} text-right tabular-nums`} style={{ whiteSpace: "nowrap" }}>
                        <FmtNum value={l.originalAmount} />
                      </td>
                      <td className={`${B} px-2 py-2 ${SIZE.tableCell} text-right tabular-nums`} style={{ whiteSpace: "nowrap" }}>
                        <FmtNum value={l.amount} />
                      </td>
                    </tr>
                  ))}
                  <tr className={`${COLOR.tableHeadBg} ${WEIGHT.bold}`}>
                    <td colSpan={5} className={`${B} px-3 py-1.5 ${SIZE.tableCell} text-right`}>TOTAL</td>
                    <td className={`${B} px-2 py-1.5 ${SIZE.tableCell} text-right tabular-nums`} style={{ whiteSpace: "nowrap" }}>
                      <FmtNum value={totalAmt} />
                    </td>
                  </tr>
                  <tr>
                    <td colSpan={6} style={{ borderLeft: "1px solid #b0b0b0", borderRight: "1px solid #b0b0b0", borderBottom: "1px solid #b0b0b0", borderTop: "none", padding: "5px 10px" }} className={SIZE.labelText}>
