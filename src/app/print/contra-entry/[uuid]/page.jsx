...
/* ── transfer-type helper ───────────────────────────────────────────────────── */
function getTransferType(lines = []) {
  const dr = lines.find((l) => l.drCr === "Dr");
  const cr = lines.find((l) => l.drCr === "Cr");
  if (!dr || !cr) return "—";
  return `${dr.accountType || "?"} → ${cr.accountType || "?"}`;
}
...
async function downloadDocx(data, uuid, qrCanvasRef) {
  ...
  const headerTable = new Table({
    ...
    rows: [new TableRow({ children: [
      ...
      new TableCell({
        children: [
          new Paragraph({ children: [run("CONTRA ENTRY", { bold: true, size: 36 })] }),
  ...
  // ── Info 2-col ────────────────────────────────────────────────────────────
  const iL = 1800; const iV = 2200; const iG = 800; const iR = 1600;
  ...
  const drLine = (data.lines || []).find((l) => l.drCr === "Dr") || {};
  const crLine = (data.lines || []).find((l) => l.drCr === "Cr") || {};

  const leftInfo  = [
    ["Voucher No",     data.voucherNo   || "—"],
    ["Entry Date",     fmt.date(data.entryDate)],
    ["Transfer Type",  getTransferType(data.lines)],
    ["Status",         data.workflowStatus || "—"],
  ];
  const rightInfo = [
    ["Debit From",  drLine.accountName || "—"],
    ["Credit To",   crLine.accountName || "—"],
  ...
  // ── Contra lines table ────────────────────────────────────────────────────
  // Columns: Dr/Cr | Particulars | Ac. Type | Opening | Debit | Credit | Closing
  const cW = [0.07, 0.30, 0.11, 0.13, 0.13, 0.13, 0.13].map((p) => Math.round(PAGE_W * p));
  cW[1] += PAGE_W - cW.reduce((a, b) => a + b, 0); // absorb rounding
  ...
  const linesTable = new Table({
    ...
    rows: [
      ...
      new TableRow({ tableHeader: true, children:
        ["Dr./Cr.", "Particulars", "Ac. Type", "Opening (₹)", "Debit (₹)", "Credit (₹)", "Closing (₹)"].map((h, i) =>
      ...
      ...lines.map((l) => {
        ...
        return new TableRow({ children: [
          ...
          numCell(l.openingBalance, cW[3], false, undefined),
      ...
      new TableRow({ children: [
        tCell("TOTAL", cW[0] + cW[1] + cW[2] + cW[3], true, totShading, "right"),
        // override: need manual column span — use a single cell spanning 4 cols
        ...(() => {
          // Build 3 remaining cols manually since docx doesn't let us split easily
          // We'll just make 4 separate cells below
          return [];
        })(),
      ]}),
    ],
  ...
  // The total row needs special handling — rebuild it properly
  const totalRow = new TableRow({ children: [
    new TableCell({
      children: [new Paragraph({ alignment: "right", children: [run("TOTAL", { bold: true, size: 20 })] })],
      columnSpan: 4,
      width: { size: cW[0] + cW[1] + cW[2] + cW[3], type: WidthType.DXA },
      shading: totShading,
      margins: { top: 50, bottom: 50, left: 70, right: 70 },
    }),
    numCell(totalDebit,  cW[4], true, totShading),
    numCell(totalCredit, cW[5], true, totShading),
    tCell("",            cW[6], false, totShading),
  ]});

  // Words row
  const wordsRow = new TableRow({ children: [
    new TableCell({
      children: [new Paragraph({ children: [
        run("Amount in Words: ", { bold: true }),
        run(amountToWordsIN(Number(data.totalAmount || 0)), { italic: true }),
      ]})],
      columnSpan: 7,
      width: { size: PAGE_W, type: WidthType.DXA },
  ...
  a.href     = url;
  a.download = `ContraEntry_${data.voucherNo || uuid}.docx`;
  a.click();
  URL.revokeObjectURL(url);
}

/* ── Main page ──────────────────────────────────────────────────────────────── */
export default function ContraEntryPrintPage() {
  ...
  useEffect(() => {
    if (!uuid) return;
    publicRequest({ url: `${API_ENDPOINTS.FINANCE.CONTRA_ENTRY.GET_BY_UUID}${uuid}` })
  ...
  const transferType = getTransferType(lines);
  const B_side = "border-l border-r border-[#b0b0b0]";

  return (
    <>
      <PrintTopBar
        title={`Contra Entry — ${data.voucherNo || ""}`}
      ...
      <div className="bg-gray-100 py-6 px-3 print:p-0 print:bg-white">
        <div
          className="bg-white max-w-[820px] mx-auto shadow-md print:shadow-none print:max-w-none"
        ...
        >
          ...
          <div className="grid px-6 pb-3" style={{ gridTemplateColumns: "50% 50%" }}>
            ...
            <div className="space-y-0.5 pl-6">
              {lines.find((l) => l.drCr === "Dr") && (
                <InfoRow label="Debit From" value={lines.find((l) => l.drCr === "Dr")?.accountName} />
              )}
              {lines.find((l) => l.drCr === "Cr") && (
                <InfoRow label="Credit To"  value={lines.find((l) => l.drCr === "Cr")?.accountName} />
              )}
              {data.remarks && (
                <p className={VAL}>
              ...
              <table className="w-full border-collapse" style={{ tableLayout: "fixed" }}>
                ...
                <thead>
                  <tr className={COLOR.tableHeadBg}>
                    {["Dr./Cr.", "Particulars", "Ac. Type", "Opening (₹)", "Debit (₹)", "Credit (₹)", "Closing (₹)"].map((h) => (
                ...
                <tbody>
                  {lines.map((l, idx) => {
                    ...
                    return (
                      <tr key={idx} className={rowBg}>
                        ...
                        <td className={`${B} px-2 py-2 ${SIZE.tableCell} text-center text-gray-600`}>
                          {l.accountType || "—"}
                        </td>
                  ...
                  <tr className={`${COLOR.tableHeadBg} ${WEIGHT.bold}`}>
                    <td colSpan={4} className={`${B} px-3 py-1.5 ${SIZE.tableCell} text-right`}>
                      TOTAL
                    </td>
