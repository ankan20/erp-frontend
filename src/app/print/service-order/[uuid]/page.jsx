...
function categoryLabel(data) {
  if (data.categoryCode === "Hire_Order")        return "HIRE ORDER";
  if (data.categoryCode === "Job_Contract_Order") return "JOB CONTRACT ORDER";
  return "WORK ORDER";
}
  ...
  return (
    <table className="w-full border-collapse" style={{ tableLayout: "fixed" }}>
      ...
      <tbody>
        ...
        {/* ── GROUP 1 ── Name / Company+C/o / SGB+Date+Phone+Website */}
        <tr>
        ...
        {/* ── GROUP 4 ── PAN/GSTIN/State/Quotation / GSTIN+State / Created by+Status */}
        <tr>
...
function renderGroups(groups) {
  return (groups || []).map((grp, gi) => (
    <div key={gi} className="mb-2">
      <p className={`${SIZE.labelText} ${WEIGHT.bold} text-gray-900`}>{gi + 1}. {grp.title}</p>
...
async function downloadDocx(data, qrCanvasRef) {
  ...
  const cW = [0.05,0.25,0.09,0.06,0.07,0.10,0.10,0.07,0.10,0.11].map(r=>Math.round(PAGE_W*r));
...
export default function ServiceOrderPrintPage() {
  ...
  useEffect(() => {
    if (!uuid) return;
    publicRequest({ url: `${API_ENDPOINTS.RESOURCE.PROCUREMENT.ORDER.PROJECT_WORK.GET_BY_UUID}/${uuid}` })
    ...
    <>
      ...
      <div className="bg-gray-100 py-6 px-3 print:p-0 print:bg-white">
        ...
        <table className="w-full max-w-[900px] mx-auto print:max-w-none" style={{ borderCollapse: "collapse", fontFamily: "var(--font-print), sans-serif" }}>
          {/* ── PRINT-ONLY RUNNING HEADER — full header repeats on every page */}
                ...
                <div className="bg-white shadow-md print:shadow-none print-body-cover">
                  ...
                  <div className="mx-6 border-x border-b border-[#b0b0b0] px-2.5 py-2">
                    ...
                    <p className={`${SIZE.labelText} text-gray-700 mt-1`}>
                      With reference to your above mentioned offer and subsequent discussion we had with you, we are pleased to place our Service Order in your favour for the following services as per terms and conditions mentioned herein.
                    </p>
                  </div>
