...
async function downloadDocx(data, qrCanvasRef) {
  ...
  const headerTable = new Table({
    ...
    rows: [new TableRow({ children: [
      ...
      new TableCell({
        children: [
          new Paragraph({ children: [run("PURCHASE VOUCHER", { bold: true, size: 36 })] }),
  ...
  const rightInfo = [
    ["Vendor Voucher No", data.vendorVoucherNo || "-"],
    ["Voucher Date",      fmt.date(data.voucherDate)],
  ...
  // Items table — 4 cols: SL (5%), CC Code (11%), CC Name & Description (62%), Amount (22%)
  // Extension rows: colSpan=2 empty (SL+CC) | col2 label | col3 value
  const cW4 = [
    Math.round(PAGE_W * 0.05),
    Math.round(PAGE_W * 0.11),
    Math.round(PAGE_W * 0.62),
  ];
  cW4.push(PAGE_W - cW4.reduce((a, b) => a + b, 0));

  const tCell = (children, w, bold = false, shading, align = "left") => {
    const paras = typeof children === "string" || typeof children === "number"
      ? [new Paragraph({ alignment: align, children: [run(String(children ?? ""), { bold, size: bold ? 20 : 18 })] })]
      : children;
    return new TableCell({ children: paras, width: { size: w, type: WidthType.DXA }, shading: shading || undefined, margins: { top: 50, bottom: 50, left: 70, right: 70 } });
  };
  ...
  const mkSummaryRow4 = (label, val) =>
    new TableRow({ children: [
      new TableCell({ children: [new Paragraph({ text: "" })], width: { size: cW4[0] + cW4[1], type: WidthType.DXA }, borders: { left: tblBorder, right: noB, top: noB, bottom: noB } }),
      new TableCell({ children: [new Paragraph({ alignment: "left",  children: [run(label, { size: 18 })] })], width: { size: cW4[2], type: WidthType.DXA }, borders: { left: tblBorder, right: noB, top: noB, bottom: noB }, margins: { top: 40, bottom: 40, left: 60, right: 0 } }),
      new TableCell({ children: [new Paragraph({ alignment: "right", children: [run(val,   { size: 18 })] })], width: { size: cW4[3], type: WidthType.DXA }, borders: { left: noB, right: tblBorder, top: noB, bottom: noB }, margins: { top: 40, bottom: 40, left: 0, right: 60 } }),
    ]});

  const summaryExtRows = [
    ...selectedGst.map(l => mkSummaryRow4(`GST @ ${l.percent}% (${l.gstType})`, fmt.number(l.gstAmount))),
    ...(discount > 0 ? [mkSummaryRow4("Discount", `- ${fmt.number(discount)}`)] : []),
    mkSummaryRow4("Round Off", roundOff === 0 ? "-" : fmt.number(roundOff)),
  ];

  const itemsTable = new Table({
    columnWidths: cW4, width: { size: PAGE_W, type: WidthType.DXA }, borders: tblBorders,
    rows: [
      new TableRow({ tableHeader: true, children: ["SL", "CC Code", "CC Name & Description", "Amount (₹)"].map((h, i) => tCell(h, cW4[i], true, headShading, "center")) }),
      ...items.map((item, idx) => new TableRow({ children: [
        tCell(idx + 1,                      cW4[0], false, undefined, "center"),
        tCell(item.ccCode,                  cW4[1]),
        tCell([
          new Paragraph({ children: [run(item.ccName || "-", { size: 18 })] }),
          ...(item.description ? [new Paragraph({ children: [run(item.description, { size: 16, color: "6B7280" })] })] : []),
        ], cW4[2]),
        tCell(fmt.number(item.basicAmount), cW4[3], false, undefined, "right"),
      ]})),
      // Total Basic Amount
      new TableRow({ children: [
        new TableCell({ children: [new Paragraph({ text: "" })], width: { size: cW4[0] + cW4[1], type: WidthType.DXA }, borders: { left: tblBorder, right: noB, top: noB, bottom: noB } }),
        new TableCell({ children: [new Paragraph({ alignment: "left",  children: [run("Total Basic Amount", { size: 18 })] })], width: { size: cW4[2], type: WidthType.DXA }, borders: { left: tblBorder, right: noB, top: noB, bottom: noB }, margins: { top: 40, bottom: 40, left: 60, right: 0 } }),
        new TableCell({ children: [new Paragraph({ alignment: "right", children: [run(fmt.number(data.basicAmount), { size: 18 })] })], width: { size: cW4[3], type: WidthType.DXA }, borders: { left: noB, right: tblBorder, top: noB, bottom: noB }, margins: { top: 40, bottom: 40, left: 0, right: 60 } }),
      ]}),
      ...summaryExtRows,
      // Total Invoice Amount
      new TableRow({ children: [
        new TableCell({ children: [new Paragraph({ text: "" })], width: { size: cW4[0] + cW4[1], type: WidthType.DXA }, borders: { left: tblBorder, right: noB, top: noB, bottom: tblBorder } }),
        new TableCell({ children: [new Paragraph({ alignment: "left", children: [run("Total Invoice Amount (Rs.)", { bold: true, size: 20 })] })], width: { size: cW4[2], type: WidthType.DXA }, shading: grayShading, borders: { left: tblBorder, right: noB, top: noB, bottom: tblBorder }, margins: { top: 50, bottom: 50, left: 60, right: 0 } }),
        new TableCell({ children: [new Paragraph({ alignment: "right", children: [run(fmt.number(data.totalInvoiceAmount), { bold: true, size: 20 })] })], width: { size: cW4[3], type: WidthType.DXA }, shading: grayShading, borders: { left: noB, right: tblBorder, top: noB, bottom: tblBorder }, margins: { top: 50, bottom: 50, left: 0, right: 60 } }),
      ]}),
      // Amount in words
      new TableRow({ children: [
        new TableCell({
  ...
  a.href = url; a.download = `PurchaseVoucher_${data.voucherNo}.docx`; a.click();
  URL.revokeObjectURL(url);
}

/* ─── Main Page ─────────────────────────────────────────────────────── */
export default function PurchaseVoucherPrintPage() {
  ...
  useEffect(() => {
    if (!uuid) return;
    publicRequest({ url: `${API_ENDPOINTS.FINANCE.PURCHASE_VOUCHER.GET_BY_UUID}${uuid}` })
  ...
  return (
    <>
      <PrintTopBar
        title={`Purchase Voucher — ${data.voucherNo}`}
        ...
        >
          ...
          <div className="grid px-6 pb-3" style={{ gridTemplateColumns: "55% 45%" }}>
            ...
            <div className="space-y-0.5 pl-6">
              <InfoRow label="Vendor Voucher No" value={data.vendorVoucherNo || "-"} />
          ...
          {/* 4 cols: SL(5%) | CC Code(11%) | CC Name & Description(62%) | Amount(22%) */}
          {/* Extension rows start at 16%: colSpan=2 (SL+CC) empty */}
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse" style={{ tableLayout: "auto" }}>
                <colgroup>
                  ...
                  <col style={{ width: "62%" }} />  {/* 2: CC Name & Description / label start */}
                  <col style={{ width: "22%" }} />  {/* 3: Amount */}
                </colgroup>
                <thead>
                  <tr className={COLOR.tableHeadBg}>
                    ...
                    <th className={`border ${COLOR.tableBorder} px-2 py-1.5 text-left ${SIZE.tableHead} ${WEIGHT.bold} text-gray-900`}>CC Name &amp; Description</th>
                ...
                <tbody>
                  {items.map((item, idx) => (
                    <tr key={idx} style={{ breakInside: "avoid" }}>
                      ...
                      <td className={`border ${COLOR.tableBorder} px-2 py-1.5 ${SIZE.tableCell}`}>
                        <div className={`${WEIGHT.medium} text-gray-900`}>{item.ccName}</div>
                        {item.description && (
                          <div className="text-gray-500 text-[10px] mt-0.5">{item.description}</div>
                        )}
                      </td>
                  ...
                  {[
                    ...
                    <tr key={i}>
                      <td colSpan={2} style={{ borderLeft: "1px solid #b0b0b0", borderRight: "none", borderTop: "none", borderBottom: "none", padding: 0 }} />
                      <td style={{ borderLeft: "1px solid #b0b0b0", borderRight: "none", borderTop: "none", borderBottom: "none", padding: "2px 6px" }}
                  ...
                  {/* ── Total Invoice Amount ── */}
                  <tr>
                    <td colSpan={2} style={{ borderLeft: "1px solid #b0b0b0", borderRight: "none", borderTop: "none", borderBottom: "1px solid #b0b0b0", padding: 0 }} />
