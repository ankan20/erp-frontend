...
async function downloadDocx(data, uuid, qrCanvasRef) {
  ...
  const headerTable = new Table({
    ...
    rows: [new TableRow({ children: [
      ...
      new TableCell({
        children: [
          new Paragraph({ children: [run("DEBIT NOTE", { bold: true, size: 36 })] }),
  ...
  const rightInfo = [
    ...
    ["Credit Note No",   data.creditNoteNo   || "-"],
    ["Credit Note Date", fmt.date(data.creditNoteDate)],
  ];
  ...
  // Items table — 6 cols: SL | CC Code | CC Name & Desc | Basic | GST % | Total
  ...
  a.href     = url;
  a.download = `DebitNote_${data.debitNoteNo || uuid}.docx`;
  a.click();
...
export default function DebitNotePrintPage() {
  ...
  useEffect(() => {
    if (!uuid) return;
    publicRequest({ url: `${API_ENDPOINTS.FINANCE.DEBIT_NOTE.GET_BY_UUID}${uuid}` })
  ...
  return (
    <>
      <PrintTopBar
        title={`Debit Note — ${data.debitNoteNo || ""}`}
        ...
        >
          ...
          <div className="grid px-6 pb-3" style={{ gridTemplateColumns: "50% 50%" }}>
            <div className="space-y-0.5">
              <InfoRow label="DN No."       value={data.debitNoteNo} />
            ...
            <div className="space-y-0.5 pl-6">
              ...
              <InfoRow label="Credit Note No"   value={data.creditNoteNo} />
              <InfoRow label="Credit Note Date" value={fmt.date(data.creditNoteDate)} />
            </div>
          </div>
