...
async function downloadDocx(data, uuid, qrCanvasRef) {
  ...
  const headerTable = new Table({
    ...
    rows: [new TableRow({ children: [
      ...
      new TableCell({
        children: [
          new Paragraph({ children: [run("PETTY CASH BUDGET", { bold: true, size: 36 })] }),
  ...
  const totalAmt  = details.reduce((s, d) => s + Number(d.budgetAmount || 0), 0);
  const cW = [0.05, 0.12, 0.33, 0.50].map((p) => Math.round(PAGE_W * p));
  ...
  const mainTable = new Table({
    ...
    borders: tblBorders,
    rows: [
      new TableRow({ tableHeader: true, children: ["Sl.", "CC Code", "CC Name & Description", "Budget Amount (₹)"].map((h, i) => tCell(h, cW[i], true, headShading, "center")) }),
  ...
  const a    = document.createElement("a"); a.href = url; a.download = `Budget_${data.budgetNo || uuid}.docx`; a.click();
...
export default function BudgetPrintPage() {
  ...
  useEffect(() => {
    if (!uuid) return;
    publicRequest({ url: `${API_ENDPOINTS.FINANCE.PETTY_CASH.BUDGET.GET_BY_UUID}${uuid}` })
  ...
  return (
    <>
      <PrintTopBar
        title={`Petty Cash Budget — ${data.budgetNo || ""}`}
        ...
        <div className="bg-white max-w-[860px] mx-auto shadow-md print:shadow-none print:max-w-none" style={{ fontFamily: "var(--font-print), sans-serif" }}>
          ...
          <div className="flex items-center px-6 pt-4 pb-3">
            ...
            <div className="flex-1 flex items-center justify-center">
              <h1 className={`${SIZE.pageTitle} ${WEIGHT.bold} tracking-widest text-gray-900 uppercase`}>Petty Cash Budget</h1>
            </div>
          ...
          <div className="grid px-6 pb-3" style={{ gridTemplateColumns: "50% 50%" }}>
            <div className="space-y-0.5">
              ...
              <InfoRow label="Frequency"    value={data.budgetFrequency} />
              <InfoRow label="Status"       value={data.workflowStatus} />
            </div>
            <div className="space-y-0.5 pl-6">
              <InfoRow label="From Date"    value={fmt.date(data.fromDate)} />
              <InfoRow label="To Date"      value={fmt.date(data.toDate)} />
              <InfoRow label="Project"      value={data.projectCode} />
              <InfoRow label="Total Amount" value={`₹ ${fmt.number(data.totalBudgetAmount || totalAmt)}`} />
            </div>
          </div>
              ...
              <table className="w-full border-collapse" style={{ tableLayout: "fixed" }}>
                ...
                <thead>
                  <tr className={COLOR.tableHeadBg}>
                    {["Sl.", "CC Code", "CC Name & Short Description", "Budget Amount (₹)"].map((h) => (
