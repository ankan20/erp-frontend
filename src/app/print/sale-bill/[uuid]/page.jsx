...
async function downloadDocx(data, qrCanvasRef) {
  ...
  const headerTable = new Table({
    ...
    rows: [new TableRow({ children: [
      ...
      new TableCell({
        children: [
          new Paragraph({ children: [run(data.mode?.toUpperCase() || "SALE INVOICE", { bold: true, size: 36 })] }),
  ...
  const leftInfo  = [
    ...
    ["Ref. Date",      fmt.date(data.referenceDate)],
    ["Project Code",   data.projectCode],
    ["Bill To",        data.billToAddress],
  ];
  const rightInfo = [
    ["Sale Order No",  data.saleOrderNo],
    ["Cert. Bill No",  data.certifiedBillNo],
    ["Abstract No",    data.billAbstractNo],
    ["Abstract Date",  fmt.date(data.billAbstractDate)],
    ["Payment Terms",  data.paymentTerms || "-"],
    ["Ship To",        data.shipToAddress],
  ];
  ...
  // Items table with summary extension
  ...
  const sideB = { left: tblBorder, right: tblBorder, top: noB, bottom: noB, insideH: noB, insideV: noB };

  const mkSummaryRow = (label, val, highlight = false) => {
    const sh = highlight ? orangeShading : undefined;
    return new TableRow({ children: [
      ...
      new TableCell({ children: [new Paragraph({ alignment: "left", children: [run(label, { bold: highlight, size: 18 })] })], width: { size: Math.floor((cW[4]) * 0.6), type: WidthType.DXA }, shading: sh, borders: { left: tblBorder, right: noB, top: noB, bottom: noB }, margins: { top: 40, bottom: 40, left: 60, right: 0 } }),
      new TableCell({ children: [new Paragraph({ alignment: "right", children: [run(val, { bold: highlight, size: 18 })] })], width: { size: cW[4] - Math.floor(cW[4] * 0.6), type: WidthType.DXA }, shading: sh, borders: { left: noB, right: tblBorder, top: noB, bottom: noB }, margins: { top: 40, bottom: 40, left: 0, right: 60 } }),
    ]});
  };
  ...
  const summaryExtRows = [
    ...
    ...(roundOff !== 0 ? [mkSummaryRow("Round Off", fmt.number(roundOff))] : []),
  ];
  ...
  const itemsTable = new Table({
    ...
    rows: [
      ...
      new TableRow({ children: [
        ...
        tCell("Total Basic Amount", cW[2], true, blueShading, "right"),
        tCell("", cW[3], false, blueShading),
        tCell(fmt.number(data.basicAmount), cW[4], true, blueShading, "right"),
      ]}),
      // GST + discount + roundOff summary rows
      ...summaryExtRows,
      ...
      new TableRow({ children: [
        ...
        new TableCell({ children: [new Paragraph({ alignment: "left", children: [run("Total Invoice Amount (Rs.)", { bold: true, size: 20 })] })], width: { size: Math.floor(cW[4] * 0.6), type: WidthType.DXA }, shading: orangeShading, borders: { left: tblBorder, right: noB, top: noB, bottom: tblBorder }, margins: { top: 50, bottom: 50, left: 60, right: 0 } }),
        new TableCell({ children: [new Paragraph({ alignment: "right", children: [run(fmt.number(data.totalInvoiceAmount), { bold: true, size: 20 })] })], width: { size: cW[4] - Math.floor(cW[4] * 0.6), type: WidthType.DXA }, shading: orangeShading, borders: { left: noB, right: tblBorder, top: noB, bottom: tblBorder }, margins: { top: 50, bottom: 50, left: 0, right: 60 } }),
      ]}),
      // Amount in words
      new TableRow({ children: [
        new TableCell({
...
export default function SaleBillPrintPage() {
  ...
  // Shared partial-border style for summary extension rows
    ...
    <>
      ...
      <div className="bg-gray-100 py-6 px-3 print:p-0 print:bg-white">
        ...
        >
          ...
          {/* ── INFO SECTION (plain 2-col, no border separations) ──────── */}
          <div className="grid px-6 pb-3" style={{ gridTemplateColumns: "55% 45%" }}>
            <div className="space-y-0.5">
              ...
              <InfoRow label="Ref. Date"      value={fmt.date(data.referenceDate)} />
              <InfoRow label="Project Code"   value={data.projectCode} />
              {data.billToAddress && (
                <p className={VAL}>
                  <span className={LBL}>Bill To</span> :{" "}
                  <span className="whitespace-pre-wrap">{data.billToAddress}</span>
                </p>
              )}
            </div>
            <div className="space-y-0.5 pl-6">
              <InfoRow label="Sale Order No"     value={data.saleOrderNo} />
              <InfoRow label="Certified Bill No" value={data.certifiedBillNo} />
              ...
              {data.shipToAddress && (
                <p className={VAL}>
                  ...
                  <span className="whitespace-pre-wrap">{data.shipToAddress}</span>
                </p>
              )}
            </div>
          </div>
              ...
              <table className="w-full border-collapse" style={{ tableLayout: "auto" }}>
                ...
                <tbody>
                  ...
                  {/* ── Summary extension rows ──
                      col 0-2 (SL+CC+Name Left = ~41%): borderLeft only, padding 0
                      col 3-4 (Name Right+HSN = ~37%):  borderLeft only, label
                      col 5   (Amount = ~22%):          borderLeft+Right only, value
                  ── */}
                  {[
                  ...
                  {/* ── Total Invoice Amount — same empty-left as extension rows, gray bg ── */}
                  <tr>
                  ...
                  {/* ── Amount in Words — full width, left+right+bottom, no top, no bg ── */}
                  <tr>
                    <td colSpan={6}
          ...
          {data.declaration && (
            <div className="px-6 pb-3">
              <p className={`${SIZE.labelText} ${WEIGHT.bold} text-gray-800 mb-0.5`}>Declaration</p>
              <p className={`${SIZE.labelText} ${WEIGHT.normal} text-gray-700 whitespace-pre-wrap leading-relaxed`}>
                {data.declaration}
              </p>
            </div>
          )}
