...
async function downloadDocx(data, qrCanvasRef) {
  ...
  // Items table: SL | Item Code | Item Name & Desc | Unit | Claim Qty | Rate | GST% | Amount
  const cW = [0.05, 0.09, 0.32, 0.07, 0.09, 0.10, 0.07, 0.21].map((p) => Math.round(PAGE_W * p));
  ...
  const mkAmtRow = (label, val, bold = false, shading) =>
    new TableRow({ children: [
      new TableCell({ children: [new Paragraph({ text: "" })], columnSpan: 5, borders: { left: solidB, right: noB, top: nilSide, bottom: nilSide, insideH: nilSide, insideV: nilSide }, width: { size: cW.slice(0, 5).reduce((a, b) => a + b, 0), type: WidthType.DXA }, margins: { top: 0, bottom: 0 } }),
      new TableCell({
        children: [new Paragraph({ alignment: AlignmentType.LEFT, children: [run(label, { bold, size: bold ? 20 : 18 })] })],
        columnSpan: 2,
        width: { size: cW[5] + cW[6], type: WidthType.DXA },
      ...
      new TableCell({
        ...
        width: { size: cW[7], type: WidthType.DXA },
        shading: shading || undefined,
        borders: { top: nilSide, bottom: nilSide, left: solidB, right: solidB, insideH: nilSide, insideV: nilSide },
        margins: { top: 40, bottom: 40, left: 70, right: 70 },
      }),
  ...
  const mkAmtTotalRow = (label, val) =>
    new TableRow({ children: [
      new TableCell({ children: [new Paragraph({ text: "" })], columnSpan: 5, borders: { top: nilSide, bottom: solidB, left: solidB, right: noB, insideH: nilSide, insideV: nilSide }, width: { size: cW.slice(0, 5).reduce((a, b) => a + b, 0), type: WidthType.DXA }, margins: { top: 0, bottom: 0 } }),
      new TableCell({
        children: [new Paragraph({ alignment: AlignmentType.LEFT, children: [run(label, { bold: true, size: 20 })] })],
        columnSpan: 2, width: { size: cW[5] + cW[6], type: WidthType.DXA },
  ...
  const itemsTable = new Table({
    ...
    rows: [
      ...
      ...items.map((item, idx) => new TableRow({ children: [
        ...
        tCell(item.claimQty,     cW[4], false, undefined, AlignmentType.RIGHT),
      ...
      new TableRow({ children: [
        ...
        tCell("TOTAL=", cW[5], true, blueShading, AlignmentType.RIGHT),
        tCell("", cW[6], false, blueShading),
        tCell(fmt.number(billedAmount), cW[7], true, blueShading, AlignmentType.RIGHT),
      ]}),
