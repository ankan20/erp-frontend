...
export default function PWOrderItemSelectionModal({ open, onClose, form }) {
  ...
  // PW uses subCategoryCodes (array), comma-joined for API query
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      try {
        ...
        // Merge with already-selected items to preserve selections on reopen
        const merged = apiItems.map((item) => {
          const existing = existingItems.find(
            (ex) => String(ex.itemCode) === String(item.itemCode)
          );

          // balanceQty = maxQty - orderedQty (null maxQty means unlimited)
          const balanceQty =
  ...
  const handleSelect = (itemCode, checked) => {
    setTempRows((prev) =>
      prev.map((i) =>
        String(i.itemCode) === String(itemCode)
          ? { ...i, selected: checked }
          : i
      )
    );
  ...
  // SUBMIT — validate then push to form items
  const handleSubmit = () => {
    ...
    for (const item of selectedRows) {
      ...
      // Only validate max if balanceQty is defined (not null = unlimited)
      if (item.balanceQty !== null && qty > item.balanceQty) {
        toast.error(
          `${item.itemName}: Qty exceeds balance qty (${item.balanceQty})`
        );
        return;
      }
    }

    // Build items — rate/gstPercent/note/location filled in the table
    ...
    // Keep items not in current modal list (different subCode items), replace matched ones
    ...
    >
      <DialogContent
        className="
          ...
          lg:min-w-[1100px] lg:max-w-[1100px]
          p-0 gap-0
        "
      >
        {/* HEADER */}
          ...
          <table className="w-full border-collapse">
            <thead className="sticky top-0 z-10 bg-gray-100">
              <tr>
                ...
                {/* Only orderQty is editable in modal */}
            ...
            <tbody>
              ...
              {!loading && !filteredItems.length && (
                <tr>
                  <td colSpan={9} className="h-[140px] text-center text-gray-500">
                    No Items Found
                  </td>
                </tr>
              )}
              ...
              {!loading &&
                filteredItems.map((item) => {
                  const qtyError = getQtyError(item);
                    ...
                    <tr key={item.itemCode}>
                        ...
                        <div className="flex justify-center">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              handleSelect(item.itemCode, checked)
                            }
                          />
                        </div>
                      </td>
                      ...
                      <td className="border p-2">
                        <QtyInput
                          ...
                          onChange={(e) =>
                            handleQtyChange(item.itemCode, e.target.value)
                          }
