...
export default function SaleCertifiedBillForm({ mode = "create", billId, onAfterSubmit }) {
    ...
    try {
      ...
      const res = await apiRequest({
        url:    mode === "create"
          ...
          : `${API_ENDPOINTS.PROJECT.SALE_CERTIFIED_BILL.UPDATE}${billId}/edit`,
  ...
  const onSubmitForApproval = async () => {
    ...
    try {
      ...
      await apiRequest({
        url:    `${API_ENDPOINTS.PROJECT.SALE_CERTIFIED_BILL.SUBMIT}${billId}/submit`,
        method: "POST",
      });
                      ...
                      <tr key={field.id} className={index % 2 === 0 ? "bg-white" : "bg-[#f5f8fc]"}>
                        ...
                        <td className="border border-[#ccc] p-0">
                          ...
                          <input
                            ...
                            className={`${getInputClass(false, disabled)} border-0 border-t border-[#ddd] rounded-none w-full h-[22px] text-[11px] px-1.5`}
                          />
                        </td>
