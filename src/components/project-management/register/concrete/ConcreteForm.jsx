...
const concreteSchema = z.object({
  referenceOrderNo:   z.string().min(1, "Reference Order No is required"),
...
const SECTIONS = [
  {
    ...
    fields: [
      { label: "Reference Order No",      name: "referenceOrderNo",   placeholder: "Enter reference order no",  type: "text"   },
      { label: "Project Sub-Location",    name: "projectSubLocation", placeholder: "Enter sub location",         type: "text"   },
      { label: "Segment / Layer",         name: "segment",            placeholder: "Enter segment",              type: "text"   },
    ],
  },
  {
    title: "Pouring Details",
    fields: [
      { label: "Pouring Date",        name: "pouringDate",      placeholder: "", type: "date" },
      { label: "Pouring Start Time",  name: "pouringStartDate", placeholder: "", type: "time" },
      { label: "Pouring Finish Time", name: "pouringEndDate",   placeholder: "", type: "time" },
    ],
  },
  {
    title: "Grade & Volume Details",
    fields: [
      { label: "Grade of Concrete",     name: "gradeConcrete",  placeholder: "e.g. M25",        type: "text"   },
      { label: "Concrete Volume (CuM)", name: "concreteVolume", placeholder: "Enter volume",     type: "number" },
    ],
  },
  {
    title: "Requisition Details",
    fields: [
      { label: "Requisition Slip No",     name: "requisitionNo", placeholder: "Enter requisition no",  type: "text" },
      { label: "Requisition By",          name: "requisitionBy", placeholder: "Enter name",             type: "text" },
      { label: "TM Vehicle Number",       name: "vehicleNumber", placeholder: "Enter vehicle number",   type: "text" },
      { label: "Batch Slip No / Challan", name: "batchNo",       placeholder: "Enter batch no",         type: "text" },
    ],
  },
];
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      if (mode === "create") {
        ...
        toast.success("Concrete register created successfully", { id: toastId });
        ...
        setIsEditing(false);
        toast.success("Concrete register updated successfully", { id: toastId });
      }
    } catch (err) {
                ...
                <div key={name}>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-[2px]">
                    ...
                    <Input
                      ...
                      className={`${getInputClass(!!errors[name], disabled)} flex-1 sm:max-w-[400px] -ml-px`}
                    />
                  </div>
                </div>
              ))}
            ...
            <div className="flex flex-wrap items-center gap-2 -ml-px">
              ...
              {!disabled && (
                ...
                >
                  ...
                  {newFileName || existingFileUrl ? "Change File" : "Choose File"}
                </button>
              )}
